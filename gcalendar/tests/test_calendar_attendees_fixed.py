...
class TestGetEventsIncludeAttendees(TestCalendarAttendees):
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_events_default_no_attendees(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_events(user_google_email=self.user_email)
        )
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_events_with_attendee_summary(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_events(user_google_email=self.user_email, include_attendees=True)
        )
        ...
        # Solo Work should not have attendee summary
        lines = result.split('\n')
        solo_work_line = next(line for line in lines if "Solo Work" in line)
        self.assertNotIn("[Attendees:", solo_work_line)
    
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_events_empty_attendee_list(self, mock_to_thread, mock_get_service):
...
class TestCreateEventWithDictAttendees(TestCalendarAttendees):
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_create_event_string_attendees_backward_compatibility(self, mock_to_thread, mock_get_service):
        ...
        self.assertEqual(call_args[1]['body']['attendees'], expected_attendees)
        self.assertIn("Successfully created event", result)
    
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_create_event_dict_attendees_basic(self, mock_to_thread, mock_get_service):
        ...
        call_args = mock_events.insert.call_args
        self.assertEqual(call_args[1]['body']['attendees'], attendees)
    
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_create_event_dict_attendees_with_properties(self, mock_to_thread, mock_get_service):
...
class TestModifyEventWithDictAttendees(TestCalendarAttendees):
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_modify_event_string_attendees(self, mock_to_thread, mock_get_service):
        ...
        update_call_args = mock_events.update.call_args
        expected_attendees = [
            {"email": "new1@example.com"},
            {"email": "new2@example.com"}
        ]
        self.assertEqual(update_call_args[1]['body']['attendees'], expected_attendees)
    
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_modify_event_dict_attendees(self, mock_to_thread, mock_get_service):
        ...
        update_call_args = mock_events.update.call_args
        self.assertEqual(update_call_args[1]['body']['attendees'], attendees)
...
class TestGetEventAttendees(TestCalendarAttendees):
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_event_attendees_detailed_info(self, mock_to_thread, mock_get_service):
        """Test get_event_attendees returns comprehensive attendee details."""
        mock_get_service.return_value = (self.mock_service, self.user_email)
        mock_to_thread.return_value = self.sample_event_with_attendees
        
        result = self.run_async(
            get_event_attendees(user_google_email=self.user_email, event_id=self.event_id)
        )
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_event_attendees_no_attendees(self, mock_to_thread, mock_get_service):
        """Test get_event_attendees with no attendees."""
        mock_get_service.return_value = (self.mock_service, self.user_email)
        mock_to_thread.return_value = self.sample_event_no_attendees
        
        result = self.run_async(
            get_event_attendees(user_google_email=self.user_email, event_id="event_no_attendees")
        )
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_get_event_attendees_minimal_info(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_event_attendees(user_google_email=self.user_email, event_id="minimal_event")
        )
...
class TestEdgeCasesAndIntegration(TestCalendarAttendees):
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_attendee_with_all_flags(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_event(user_google_email=self.user_email, event_id="all_flags")
        )
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_very_long_attendee_list(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_event_attendees(user_google_email=self.user_email, event_id="large_event")
        )
    ...
    @patch('auth.google_auth.get_authenticated_google_service')
    @patch('gcalendar.calendar_tools.asyncio.to_thread')
    def test_special_characters_in_names(self, mock_to_thread, mock_get_service):
        ...
        result = self.run_async(
            get_event(user_google_email=self.user_email, event_id="special_chars")
        )
